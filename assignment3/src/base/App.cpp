...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		// This is very simple...glBegin(GL_POINTS);
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for (const auto& sv : source_vertices) {
		...
		Mat4f compositeTransfroms = Mat4f(0);

		for (int i = 0; i < WEIGHTS_PER_VERTEX; i++)
			compositeTransfroms += sv.weights[i] * ssd_transforms[sv.joints[i]];
		v.position = compositeTransfroms * sv.position;
		v.normal = (compositeTransfroms * Vec4f(sv.normal, 0)).getXYZ();
		v.color = sv.color;
